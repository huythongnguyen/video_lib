...
class HttpClient:
    ...
    def download(url: str, dest_path, headers: Optional[dict] = None) -> None:
        ...
        # On Windows, use extended-length paths to avoid MAX_PATH failures.
        dest = Path(dest_path) if not isinstance(dest_path, Path) else dest_path
        dest.parent.mkdir(parents=True, exist_ok=True)

        dest_str = str(dest.resolve())
        if os.name == "nt" and len(dest_str) >= 250:
            # Use Win32 extended-length path prefix.
            if dest_str.startswith("\\\\"):
                dest_str = "\\\\?\\UNC\\" + dest_str[2:]
            else:
                dest_str = "\\\\?\\" + dest_str
