...
Usage:
    python -m video_lib.main <job_file_path> [options]

Example:
    python -m video_lib.main contents/books/Meaningful-to-Behold/14_Effort/08_EXAMININGTHECAUSEOFINDOLENCE/job.json
"""
import sys
import argparse
...
def main():
    ...
    # Get available voices and styles for help text (using enum names)
...
Examples:
    ...
    python -m video_lib.main path/to/job.json --voice VAN_ANH --style COMPASSIONATE

    # Use Claude Code CLI instead of Gemini
    python -m video_lib.main path/to/job.json --llm claude

    # Force regeneration (ignore cache)
    python -m video_lib.main path/to/job.json --force

    # Generate English content
    python -m video_lib.main path/to/job.json --language English

Recommended Calm/Teaching Voices (use enum names):
    {', '.join(buddhist_voices)}

Available Content Styles (use enum names):
    {', '.join(all_styles)}
        """
    )
    ...
    parser.add_argument(
        "--voice",
        default=None,
        help=f"TTS voice enum name (default: HO_MIN_MANG). Recommended: {', '.join(buddhist_voices[:3])}"
    )
    ...
    # Get project root (2 levels up from this file: video_lib -> src -> root)
